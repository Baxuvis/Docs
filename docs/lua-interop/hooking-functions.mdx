...
Aha! There's a `BP_OtomoPalHolderComponent`, which is responsible for storing your party pals and spawning them when requested.

After looking through it's functions, we can find one with a signature like this:
